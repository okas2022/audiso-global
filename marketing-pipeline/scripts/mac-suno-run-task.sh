...
if bash "${ROOT}/scripts/mac-create-audiso-music-mydrive.sh" "$TRACK" 2>/dev/null; then
  echo "[suno-task] Drive Finder folder OK — 내 드라이브 → Audiso Music"
elif bash "${ROOT}/scripts/mac-music-drive-desktop-sync.sh" "$TRACK" 2>/dev/null; then
  echo "[suno-task] Drive desktop sync OK"
else
  echo "[suno-task] Drive folder skipped — run: bash scripts/mac-create-audiso-music-mydrive.sh" >&2
fi
