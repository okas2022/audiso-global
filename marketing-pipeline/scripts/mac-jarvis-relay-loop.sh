...
if [[ -d "${GLOBAL}/.git" ]]; then
  ...
  if git rev-parse --verify "origin/${MUSIC_BRANCH}" >/dev/null 2>&1; then
    git checkout "origin/${MUSIC_BRANCH}" -- \
      marketing-pipeline/pipeline_data/jarvis_memory/mac_tasks/pending-suno-generate-004.json \
      marketing-pipeline/pipeline_data/jarvis_memory/mac_tasks/pending-suno-generate-004-now.json \
      ...
      marketing-pipeline/scripts/mac-suno-bootstrap-and-run.sh \
      marketing-pipeline/scripts/music-drive-upload.sh \
      marketing-pipeline/scripts/music-drive-upload.py \
      2>/dev/null || true
    chmod +x marketing-pipeline/scripts/mac-music-phone-pipeline.sh \
      marketing-pipeline/scripts/mac-music-drive-desktop-sync.sh \
      marketing-pipeline/scripts/mac-suno-bootstrap-and-run.sh \
